...
@section('content')
    ...
    {{-- Scope switch: Romania by default, Europe opens a country picker. --}}
    <div class="mb-6 inline-flex rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-900">
        @php
            $tabs = ['romania' => 'Romania', 'europe' => 'Europe'];
        @endphp
        @foreach ($tabs as $key => $label)
            <a href="{{ route('home', $key === 'europe' ? ['scope' => 'europe'] : []) }}"
    ...
    @if ($scope === 'europe')
        <div class="mb-8">
            <h2 class="mb-3 text-sm font-medium text-slate-500 dark:text-slate-400">Select a country</h2>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($countries as $code => $name)
                    <a href="{{ route('home', ['scope' => 'europe', 'country' => $code]) }}"
                       class="group flex items-center justify-between rounded-xl border p-4 transition
                              {{ $selectedCountry === $code
                                    ? 'border-sky-500 bg-sky-50 dark:border-sky-500 dark:bg-sky-950/40'
                                    : 'border-slate-200 bg-white hover:border-sky-400 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-sky-600' }}">
                        <span class="flex items-center gap-3">
                            <span class="rounded-md bg-slate-100 px-2 py-1 font-mono text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ $code }}</span>
                            <span class="text-sm font-medium">{{ $name }}</span>
                        </span>
                        <span class="text-xs text-slate-400">{{ $counts[$code] ?? 0 }} airports</span>
                    </a>
                @endforeach
            </div>
        </div>
    ...
    @if ($selectedCountry)
        <h2 class="mb-3 text-sm font-medium text-slate-500 dark:text-slate-400">
            {{ $countries[$selectedCountry] }} — {{ $airports->count() }} airports
        </h2>
    ...
    @elseif ($scope === 'europe')
        <p class="text-sm text-slate-500 dark:text-slate-400">Pick a country above to list its airports.</p>
    @endif
@endsection
